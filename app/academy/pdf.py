...
def _field(label: str, value: str) -> str:
    if not value:
        return ""
    return (
        f'<div class="field"><span class="k">{escape(label)}</span>'
...
def render_fiche_html(fiche: NeedFiche, project_title: str | None = None) -> str:
    ...
    deliverables = details.get("deliverables") if isinstance(details.get("deliverables"), list) else []
  ...
  .chips span {{ display: inline-block; border: 1px solid #DDD; border-radius: 12px; padding: 2px 10px; font-size: 11px; margin: 0 4px 4px 0; }}
  ...
  {(f'<h2>Compétences attendues</h2><div class="chips">' + "".join(f"<span>{escape(str(s))}</span>" for s in skills) + "</div>") if skills else ""}
