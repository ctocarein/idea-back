...
def test_store_source_pdf_sets_key():
    deck, st = _deck(), FakeStorage()
    store_deck_source(st, deck, "application/pdf", b"%PDF-1.4 ...")
...
def test_store_source_image_sets_key():
    deck, st = _deck(), FakeStorage()
    store_deck_source(st, deck, "image/png", b"\x89PNG")
    assert deck.source_key is not None


def test_store_source_pptx_not_presentable():
    # Le PPTX n'est pas rendu par le navigateur → pas de fichier source visuel.
    deck = _deck()
    store_deck_source(FakeStorage(), deck, PPTX, b"PK...")
    assert deck.source_key is None


def test_store_source_without_storage_is_noop():
    deck = _deck()
    store_deck_source(None, deck, "application/pdf", b"%PDF")
    assert deck.source_key is None


def test_deck_to_out_exposes_presigned_url():
    deck = _deck(source_key="pitch-decks/abc/source", source_content_type="application/pdf")
    out = deck_to_out(deck, [], FakeStorage())
    assert out.file_url == "https://signed/pitch-decks/abc/source"
    assert out.content_type == "application/pdf"


def test_deck_to_out_no_source_no_url():
    out = deck_to_out(_deck(), [], FakeStorage())
